...
def draw_bboxes(image, bboxes, font_size=1, thresh=0.5, colors=None):
  ...
  'Kayak', 'Sail boat', 'Swimming person', 'Flying bird/plane', 'Other']
    ...
    for bbox in bboxes:
        ...
        cat_id = int(bbox[-3])
        cat_size  = cv2.getTextSize(smd_class_name[cat_name]+"0.99 - ID: 100", cv2.FONT_HERSHEY_SIMPLEX, font_size, 3)[0]
        ...
        if bbox[1] - cat_size[1] - 2 < 0:
            ...
            categories.append(smd_class_name[cat_name])
            cv2.putText(image, smd_class_name[cat_name]+" {:.1f} - ID: {}".format(conf,cat_id),
